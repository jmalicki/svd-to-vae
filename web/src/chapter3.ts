...
app.innerHTML = `
  <header>
    <p class="chapter-nav">
      <a href="./chapter2.html">← Squeezing a face through a few numbers</a>
    </p>
    ...
    <p class="lede">
      Chapter&nbsp;2 squeezed a face through $k$ numbers and decoded it again — that was compression.
  ...
  <section class="panel demo-hero" aria-label="Noise demo">
    ...
    <div class="controls face-controls">
      <label>
        Bottleneck $k$ (fixed from ch.&nbsp;2 idea)
  ...
  <section class="panel" aria-label="Random gallery">
    ...
    <p class="hint">
      Each tile: pick a random training $z$, add noise, decode. Same warp-aligned SVD as chapter&nbsp;2.
    </p>
  ...
  <footer class="chapter-footer">
    <a href="./chapter2.html">← Back: squeezing faces</a>
    <a href="./">Home: SVD via gradient descent</a>
  </footer>
`;
...
void (async () => {
  try {
    ...
    el.loadStatus.textContent = `Warping ${manifest.files.length} faces…`;
